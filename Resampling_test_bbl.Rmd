...
resample_RSG <- tibble(RSG1_no_SD = sapply(rep(sample_size, N_SAMPLES), method1, GlobalRs$Rs),
                       RSG2_cal_SD = sapply(rep(sample_size, N_SAMPLES), method2, GlobalRs$Rs, GlobalRs$SD),
                       RSG3_max_SD = sapply(rep(sample_size, N_SAMPLES), method3, GlobalRs$Rs, GlobalRs$SD)
                       )

# resample gross primary production (GPP)
resample_GPP <- tibble(GPP1_no_SD = sapply(rep(sample_size, N_SAMPLES), method1, GPP$GPP),
                       GPP2_cal_SD = sapply(rep(sample_size, N_SAMPLES), method2, GPP$GPP, GPP$SD),
                       GPP3_max_SD = sapply(rep(sample_size, N_SAMPLES), method3, GPP$GPP, GPP$SD)
                       )
```
