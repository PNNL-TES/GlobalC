...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown, `Upscaled-Rs` = Rs_raw) %>% 
  gather(Rs_type) ->
  rs_figure_data
...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown2, `Upscaled-Rs` = Rs_raw) %>% 
  gather(Rs_type) ->
  rs_figure_data
...
ratio_results %>%
  select(`Rs[GPP]` = Rs, `Upscaled-Rs` = Rs_raw) %>% 
  gather(Rs_type) ->
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) + geom_point(alpha = 0.05) + geom_density2d() +
  scale_color_discrete("Bootstrap\ndata source", labels = c(expression(GPP), expression(R[S]))) +
  xlab(expression(GPP~(Pg~C~yr^-1))) +
  ylab(expression(R[S]~(Pg~C~yr^-1))) +
  ...
  annotate("segment", x = mean(rs_results$GPP_raw), xend = 75, 
           ...
           arrow = myarrow, color = "#F8766D", linetype = 2) +
  ...
  annotate("segment", x = mean(gpp_results$GPP2), xend = mean(gpp_results$GPP2), 
           ...
           arrow = myarrow, color = "#00BFC4", linetype = 2)
print(p_joint)
ggsave("outputs/Figure3_not_sure.png", width = 6, height = 4)
```
