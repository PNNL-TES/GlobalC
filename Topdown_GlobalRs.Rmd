...
cmip6_data_file <- "/Users/d3x290/Downloads/cmip6/output/1.cmip6_global_monthly_means.csv"
if(file.exists(cmip6_data_file)) {
  read.csv(cmip6_data_file, stringsAsFactors = FALSE) %>% 
    as_tibble() %>% 
    filter(model != "IPSL-CM6A-LR") %>% 
    select(model, experiment, ensemble, variable, time, value, LandArea) %>% 
    mutate(year = floor(time / 10000)) %>% 
    group_by(variable, model, experiment, ensemble, year) %>% 
  ...
  model_ratios %>% 
    mutate(GPP = gpp, Source = paste0("CMIP6\n(N = ", nrow(model_ratios), ")"), Type = "Models") ->
    cmip6
} else {
