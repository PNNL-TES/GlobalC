...
FlFsFr <- read.csv(file.path(DATA_DIR, 'FlFsFrSummary.csv'))
GPP <- read.csv(file.path(DATA_DIR, 'GlobalGPP_Sum.csv'))
NPP <- read.csv(file.path(DATA_DIR, 'ITONPP.CSV'))
GlobalRs <- read.csv(file.path(DATA_DIR, 'GlobalRs.csv'))
...
plot_froot <- function (sdata) {
  ...
  print(median(sdata$Fr, na.rm = TRUE) / 
          (median(sdata$Fl, na.rm = TRUE) + median(sdata$Fs, na.rm = TRUE) + 
             median(sdata$Fr, na.rm = TRUE))*100)
  se <- sd(sdata$Fr, na.rm = TRUE) / nrow(subset(sdata, !is.na(sdata$Fr)))
  paste0("se=", se) %>% print()
  ...
  print(median(sdata$Fs, na.rm = TRUE) / (median(sdata$Fl, na.rm = TRUE) + 
                                            median(sdata$Fs, na.rm = TRUE) + 
                                            median(sdata$Fr, na.rm = TRUE)) * 100)
  se <- sd(sdata$Fs, na.rm = TRUE) / nrow(subset(sdata, !is.na(sdata$Fs)))
  paste0("se=", se) %>% print()
  ...
  print(median(sdata$Fl, na.rm = TRUE) / (median(sdata$Fl, na.rm = TRUE) + 
                                            median(sdata$Fs, na.rm = TRUE) + 
                                            median(sdata$Fr, na.rm = TRUE)) * 100)
  se <- sd(sdata$Fl, na.rm = TRUE)/nrow(subset(sdata, !is.na(sdata$Fl)))
  paste0("se=", se) %>% print()
...
rs_results <- tibble(
  ...
  Ra1 = GPP * RaGpp,
  Ra2 = ifelse(GPP < NPP, 0, (GPP - NPP)),   # BBL: what's this comma? Why doesn't it cause an error?
  Ra = (Ra1 + Ra2) / 2,
...
```{r}
# about 27% chance Rs will be between (80-92)
