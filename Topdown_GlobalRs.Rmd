...
We used `r nrow(GPP) published GPP values`
...
gpp_results <- tibble(
  ...
  gpp11 = sample(GPP[is.na(GPP$SD),]$GPP, n_samp, replace = TRUE), # for those do not have SD information, take 50000 samples using boosting 
  GPP_raw =  (gpp1 + gpp2 + gpp3 + gpp4 + gpp5 + gpp6 + gpp7 + gpp8 + gpp9 + gpp10 + gpp11) / 11, # scenario 1: calculate 
...
rs_results <- tibble(
  ...
  gpp8 = rnorm(n = n_samp, mean = 147, sd = 16),
  gpp9 = rnorm(n = n_samp, mean = 119, sd = 6), # same as above
  gpp10 = rnorm(n = n_samp, mean = 122, sd = 25), # same as above
  gpp11 = sample(GPP[is.na(GPP$SD),]$GPP, n_samp, replace = TRUE), # for those do not have SD information, take 50000 samples using boosting 
  GPP =  (gpp1 + gpp2 + gpp3 + gpp4 + gpp5 + gpp6 + gpp7 + gpp8 + gpp9 + gpp10 + gpp11)/11,
...
ggplot(rs_results) + aes(Rs_topdown) + geom_density(col = "gray", fill = rgb(1,0,1, alpha = 0.25)) + 
...
ggsave("outputs/Figure3.jpg", width = 6, height = 4, dpi = 300, units = "in")
