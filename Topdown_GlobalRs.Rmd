...
IGBP %>% 
  ...
  summarise(rel_area = sum(abs(Latitude) / 180 * pi)) -> 
  IGBP_summary_ecosystem

srdb_v4_ratio <- left_join(srdb_v4, IGBP_summary_ecosystem, by = "Ecosystem_type")

Rs_to_GPP_unweighted <- with(srdb_v4, mean(Rs_annual / GPP, na.rm = TRUE))
Rs_to_GPP_weighted <- with(srdb_v4_ratio, weighted.mean(Rs_annual / GPP, w = rel_area, na.rm = TRUE))
...
p1_gpp3 <- annotations(gpp_ratio_figure,
                       unique(gpp_ratio_figure_data$GPP_type), 
                       gpp_qt, gpp_qt_raw, 200)
print(p1_gpp3)
```
...
ratios %>% 
  group_by(Source) %>% 
  summarise(Ratio = round(median(Ratio), 3)) %>%
