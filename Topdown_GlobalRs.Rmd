...
Terrestrial plants sequester carbon from the atmosphere through photosynthesis (i.e., gross primary productivity, GPP); roughly the same amount of carbon is decomposed and returned back to the atmosphere by respiratory fluxes, in particular the global soil-to-atmosphere CO[2] flux (soil respiration, R[S]). 
Significant progress in quantifying and constraining global GPP has been achieved using top-down approaches, i.e. satellite data products; conversely, global R[S] estimates are generally derived by upscaling site-scale measurements. 
The uncertainty in these large flux estimates is poorly constrained, however, and their consistency has not been previously evaluated.

We partitioned historical global GPP estimates into their net primary production (NPP) and autotrophic respiration components and calculated the implied resulting global R[S]; we also used global R[S] estimates to estimate the implied global GPP.
Here we show that the R[S] implied by global GPP (`r round(mean(rs_results$Rs_topdown2), 0)`±`r round(sd(rs_results$Rs_topdown2), 0)` Pg C yr^{-1}) is fundamentally inconsistent (X%, P = 0.xxx) with bottom-up global R[S] estimates (x ± y). 
Similarly, partitioning bottom-up R[S] values into shoot and root respiration, resulted in GPP values significantly different than top-down estimates. 
We also show that the global R[S] to global GPP ratio is consistent with neither site-level measurements of the R[S]:GPP ratio, nor with CMIP6 model results.

Our findings demonstrate a significant inconsistency between bottom-up and top-down based GPP and RSG estimates, which have a ≥20 Pg C gap between them.
...
# Each row get the closest latitude and longitude values to AGB and SOC data
# AGB$Latitude[which.min(abs(61.85 - AGB$Latitude))]
# Froot %>% mutate(Latitude = round(Latitude*2, 0)/2 - 0.25, Longitude = round(Longitude*2, 0)/2-0.25) -> Froot
# there are no clear relationship between Froot and above-ground biomass or soil organic carbon (SOC)
# left_join(Froot, AGB) %>% ggplot() + aes(RASTERVALU, Froot) + geom_point()
