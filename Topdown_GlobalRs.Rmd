...
# We're adding same annotations to each figure so use a function
annotations <- function(figure, types, x_qt, x_qt_raw, xpos) {
  lbl <- function(qt_data) {
  ...
  ann_dat <- tibble(Type = types,
                    ...
                    x = xpos, 
                    y = c(0.035, 0.025),
                    label = c(lbl(x_qt), lbl(x_qt_raw)))
  figure + geom_text(data = ann_dat,
                     aes(x = x, y = y, label = label, color = Type),
                     hjust = 0, show.legend = FALSE)
}

annotations(gpp_base_figure, unique(gpp_figure_data$GPP_type), gpp_qt, gpp_qt_raw, 150)
...
annotations(gpp_base_figure %+% gpp_figure2_data,
            unique(gpp_figure2_data$GPP_type),rsg_qt, gpp_qt_raw)
```
...
annotations(rs_base_figure, types = unique(rs_figure_data$Rs_type), rsg_qt, rsg_qt_raw, 100)
...
annotations(rs_base_figure %+% rs_figure_data, 
...
p1 <- annotations(gpp_base_figure, 
