...
cmip6_local_data_file <- "cmip6/output/cmip6-ratio-RS2GPP-fromgridcell.csv"
if(file.exists(cmip6_local_data_file)) {
  read.csv(cmip6_local_data_file) %>% 
    as_tibble() %>% 
    filter(year >= 2005) ->
    cmip6_local
  
  cmip6_local %>% 
    mutate(Source = paste0("Local CMIP6\n(N = ", nrow(cmip6_local), ")"),
           Scale = "Local",
           Type = "Models") %>% 
    select(Source, Scale, Type, Ratio = value) ->
    cmip6_local

  # ... process it
} else {
  cmip6_local <- data.frame(Source = "Local CMIP6\n(N = 0)", 
                           Scale = "Local",
                           Type = "Models", 
                           Ratio = NA_real_,
...
bind_rows(topdown, srdb, fluxnet, cmip_global) %>% 
  mutate(Ratio = Rs / GPP) %>% 
  bind_rows(cmip6_local) %>% 
  mutate(Source = factor(Source, unique(Source)[c(2, 3, 5, 4, 1)])) ->
  ratios
...
p_ratios <- ggplot(ratios, aes(Source, Ratio, color = Type)) + 
  ...
  geom_boxplot(color = "black", alpha = 0.25) +
