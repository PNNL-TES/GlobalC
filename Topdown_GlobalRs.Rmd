...
AGB <- read.table(paste0(DATA_DIR,'/', 'GLC2000_Point.txt'), header = TRUE, sep = ",")
...
SOC <- read.table(paste0(DATA_DIR,'/', 'SOC_Point.txt'), header = TRUE, sep = ",")
...
gpp_results <- tibble(
  ...
  # Step 6. calculation ********************
  # Rc = replicate( n_rep, var_RC$RC_annual, n_samp, replace = TRUE) %>% mean() ), # senario 0: Rc not separate into different ecosystems, not used
  ...
  # Senario 2: Rc separate into 4 groups: agriculture, forest, grassland, and rest (all other ecosystems with obs<40, take samples from all RC records)
  ...
  # senario1: Froot not separate into different ecosystems
  ...
  # Senario 2: separate into 2 ecosystems and weighted by area
...
# scenatio 1. RaGPP ratio not separated
...
```{r}
# senario 2. RaGPP ratio separated into 2 groups
...
# senario 2, RC separate into 4 groups, and Froot separate into 2 groups
...
ggplot(gpp_results) + aes(GPP2) + 
  ...
  annotate("text", x = 150, y = 0.025, label = paste0("2.5%,50%,97.5% quantile (", gpp_qt_raw[1],", ", gpp_qt_raw[2], ", ", gpp_qt_raw[3], ")")
           , hjust = 0, col = rgb(0, 1, 1)) +
  # annotate("text", x = 150, y = 0.045, label = expression("Rroot-to-R"[S]~"separated into 4 groups")
  #          , hjust = 0) +
...
ggplot(rs_results) + aes(Rs_topdown2) + 
  ...
  geom_density(aes(Rs_raw), col = "gray", fill = rgb(0, 1, 1, alpha = 0.5)) +
  # geom_vline(xintercept = c(quantile (rs_results$Rs_raw, c(0.025, 0.5, 0.975))),col = "red", linetype = "dotted", size = 1.5) +
  # annotate("text", x = 100, y = 0.08, label = expression("Rroot-to-Ra ratio separated into 4 groups")
  #          , hjust = 0) +
