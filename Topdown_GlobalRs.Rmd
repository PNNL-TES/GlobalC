...
plot_froot <- function (sdata) {
  ...
  Fl_plot <- ggplot(sdata, aes(x = IGBP2, y=Froot)) + geom_violin() +
    ...
    ylab("Rroot-to-Ra-ratio")
  
  print(Fl_plot)
}

plot_froot (Froot)
ggsave("outputs/FigureS2.jpg", width = 6, height = 4, dpi = 300, units = "in")
```
...
sub_srdb %>% mutate(IGBP2 = case_when( Ecosystem_type == "Agriculture" ~ "Agriculture"
                                       ...
                                       , TRUE ~ "Other" )) -> sub_srdb
sub_srdb %>% group_by(IGBP2) %>% summarise(count = n())

plot_Rroot_Rs (GlobalRs, sub_srdb, NPP) 
ggsave("outputs/FigureS3.jpg", width = 8, height = 6, dpi = 300, units = "in")
```
...
```{r}
# prepare data for bottom-up boostrapping
...
```{r}
# Bottom-up approache to estimate GPP
...
gpp_results %>% ggplot() + aes(GPP) + 
  ...
  annotate("text", x = 150, y = 0.035, label = paste0("2.5%, 50%, 97.5% quantile (", var_CI[1],", ", var_CI[2], ", ", var_CI[3], ")")
           , hjust = 0, col = rgb(1,0,1)) +
  annotate("text", x = 150, y = 0.025, label = paste0("2.5%,50%,97.5% quantile (", var_CI_raw[1],", ", var_CI_raw[2], ", ", var_CI_raw[3], ")")
...
p_gpp_less_than_133 <- pnorm ((134 - y) / (s))
p_gpp_less_than_107 <- pnorm ((111 - y) / (s))
...
```{r}
# prepare data for top-down approach
...
ggplot(rs_results) + aes(Rs_topdown) + 
  ...
  annotate("text", x = 100, y = 0.04, label = paste0("2.5%, 50%, 97.5% quantile (", var_CI[1],", ", var_CI[2], ", ", var_CI[3], ")")
           , hjust = 0, col = rgb(1,0,1)) +
  annotate("text", x = 100, y = 0.03, label = paste0("2.5%, 50%, 97.5% quantile (", var_CI_raw[1],", ", var_CI_raw[2], ", ", var_CI_raw[3], ")")
           , hjust = 0, col = rgb(0,1,1)) +
...
ggplot(rs_results) + aes(Rs_topdown2) + 
  ...
  #          , hjust = 0) +
  annotate("text", x = 100, y = 0.06, label = paste0("2.5%, 50%, 97.5% quantile (", var_CI[1],", ", var_CI[2], ", ", var_CI[3], ")")
           , hjust = 0, col = rgb(1,0,1)) +
  annotate("text", x = 100, y = 0.04, label = paste0("2.5%, 50%, 97.5% quantile (", var_CI_raw[1],", ", var_CI_raw[2], ", ", var_CI_raw[3], ")")
           , hjust = 0, col = rgb(0,1,1)) +
  ylab(expression(count)) +
...
```{r}
# assuming Rs follows a normal distribution, with mean = y and sd = s, we can calculate the probability of Rs following (80-92) 95%CI
...
bind_rows(topdown, srdb, fluxnet) %>% 
  mutate(Ratio = Rs / GPP) ->
  ratios
