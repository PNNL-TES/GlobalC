...
gpp_figure_data %>% 
  ...
  scale_fill_discrete("", labels = c(expression(Canonical~GPP),expression(GPP[Rs]~"            "))) + 
  ylab("Density") +
...
```{r joint-plot, results='show', fig.width=5, fig.height=4}
gpp_results %>% 
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) + 
  geom_point(pch = 20, size = 1, alpha = 0.05) + 
