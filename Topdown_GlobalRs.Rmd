...
gpp_results %>%
  select(`GPP[Rs]` = GPP, `Canonical GPP` = GPP_raw) %>%
  gather(GPP_type) ->
gpp_figure_data
...
gpp_figure_data %>%
  ...
  scale_fill_manual("",
    labels = c(expression(Canonical~GPP), expression(GPP[Rs])),
...
gpp_results %>%
  select(`GPP[Rs]` = GPP2, `Canonical-GPP` = GPP_raw) %>%
  gather(GPP_type) ->
gpp_figure2_data
...
pairsdata <- rename(pairsdata, "Canonical Rs" = Rs_raw, "Rroot : Rs ratio" = Rc, "Rroot : RA ratio" = Froot)
# pdf("outputs/FigureS8 - Boots GPP pairs.pdf", width = 8, height = 6)
pairs(pairsdata, col = pairs_colors, main = expression(GPP[Rs]))
# dev.off()
# get mean Rroot:Rs of below of above the intercetion (126.7 Pg)
gpp_results %>%
  select(GPP, Rc) %>%
  mutate(GPP_group = ifelse(GPP < gpp_ip, "black", "red")) %>%
  group_by(GPP_group) %>%
  summarize(mean(Rc))

# get mean Rroot:Ra of below of above the intercetion (126.7 Pg)
gpp_results %>%
  select(GPP, Froot) %>%
  mutate(GPP_group = ifelse(GPP < gpp_ip, "black", "red")) %>%
  group_by(GPP_group) %>%
...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown, `Canonical Rs` = Rs_raw) %>%
  gather(Rs_type) ->
rs_figure_data
...
rs_figure_data %>%
  ...
  scale_fill_manual("",
    labels = c(expression(Canonical ~ Rs), expression(Rs[GPP])),
...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown2, `Canonical-Rs` = Rs_raw) %>%
  gather(Rs_type) ->
rs_figure_data
...
ratio_results %>%
  select(`Rs[GPP]` = Rs, `Canonical-Rs` = Rs_raw) %>%
  gather(Rs_type) ->
...
pairsdata <- rename(pairsdata, "Canonical GPP" = GPP_raw, "Rroot : RA ratio" = Froot_rest, "RA : GPP ratio" = RaGpp_rest)
# pdf("outputs/FigureS9 - Boots Rs pairs.pdf", width = 8, height = 6)
pairs(pairsdata, col = pairs_colors, main = expression(Rs[GPP]))
# dev.off()

# get mean Rroot:Rs, NPP, canonical-GPP of below of above the intercetion (78.11 Pg)
rs_results %>%
  select(Rs_topdown, Froot_rest, NPP, GPP_raw) %>%
  mutate(Group = ifelse(Rs_topdown < rs_ip, "black", "red")) %>%
  group_by(Group) %>%
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) +
  ...
  annotate("text",
    x = c(100, 151), y = 47.5, size = 4, hjust = "inward", fontface = "bold",
    label = c(expression(Canonical ~ GPP), expression(GPP[Rs]))
  ) +
  annotate("text",
    x = 70, y = c(65, 81), size = 4, hjust = "inward", fontface = "bold",
    label = c(expression(Rs[GPP]), expression(Canonical ~ R[S])),
    angle = 90
  )
print(p_joint)
...
topdown$Source <- paste0("Global Rs\nand GPP\n(N = ", nrow(topdown), ")")
topdown$Type <- "Global"
