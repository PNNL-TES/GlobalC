...
GPP %>% select(Group, Flag) %>% count(Group, Flag)
```
...
var_RaGpp_mf <- var_RaGpp %>% filter(IGBP2 == "Mixed") %>% pull(RaGPP_ratio)
...
# Ra2 = GPP - NPP
rs_results %>% select(`RA = GPP * RA:GPP ratio` = Ra1, `RA = GPP - NPP` = Ra2) %>% 
  gather(RA_calculation) %>% 
  ggplot(aes(value, fill = RA_calculation)) +
  geom_density(stat = 'density', alpha = 0.5) +
  # theme(legend.position = c(0.15, 0.35)) +
  coord_cartesian(xlim = c(10, 100)) +
  ylab("Density") + xlab(expression(R[A]~(Pg~C~yr^{-1}))) +
  scale_fill_discrete("", labels = c(expression(R[A]~"="~GPP~"× R"[A]~":"~"GPP ratio"), 
...
rs_results %>% select(Rs_topdown, Froot_rest, NPP, GPP_raw) %>% 
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) + 
  ...
  scale_color_discrete("", labels = c(expression(Canonical~GPP~and~R[S]~implied~by~GPP~"("~Rs[GPP]~")"), 
  ...
  annotate("segment", x = mean(rs_results$GPP_raw), xend = mean(rs_results$GPP_raw), 
           y = 50, yend = mean(rs_results$Rs_topdown2), 
           color = "#F8766D", linetype = 1) +
  annotate("segment", x = mean(rs_results$GPP_raw), xend = 75, 
           y = mean(rs_results$Rs_topdown2), yend = mean(rs_results$Rs_topdown2), 
           arrow = myarrow, color = "#F8766D", linetype = 1) +
  annotate("segment", x = 75, xend = mean(gpp_results$GPP2), 
           y = mean(gpp_results$Rs_raw), yend = mean(gpp_results$Rs_raw), 
           color = "#00BFC4", linetype = 1) +
  annotate("segment", x = mean(gpp_results$GPP2), xend = mean(gpp_results$GPP2), 
           y = mean(gpp_results$Rs_raw), yend = 50, 
           arrow = myarrow, color = "#00BFC4", linetype = 1) +
  annotate("text", x = c(100,151), y = 47.5, size = 4, hjust = "inward", fontface = "bold",
           label = c(expression(Canonical~GPP), expression(GPP[Rs]))) +
  annotate("text", x = 70, y = c(65, 81), size = 4, hjust = "inward", fontface = "bold", 
...
if(file.exists(cmip6_global_data_file)) {
  ...
  cmip_global <- data.frame(Source = "CMIP6\n(N = 0)", Scale = "Global",
...
if(file.exists(cmip6_local_data_file)) {
  ...
  cmip_local <- data.frame(Source = "CMIP6\n(N = 0)", 
...
ratios %>% 
  group_by(Source) %>% 
  summarise(Ratio_median = round(median(Ratio, na.rm = TRUE), 3),
