...
# var_rest <- var_RC %>% filter (Ecosystem_type != "Forest") # dataset contain RC records for non-forest of ecosystem
var_Rs <- GlobalRs %>% filter(!is.na(Rs) & is.na(SD) ) # dataset contain global Rs estimates (without standard deviation information)
...
# Create the base data frame used in both Rs and GPP bootstrapping
bootstrap_data <- tibble(
  # Step 1. Prepare Global Rs data
  # Rs = replicate( n_rep, sample(var_Rs$Rs, N_SAMPLES, replace = TRUE) %>% mean() ),
  # Rs1 = rnorm(n = N_SAMPLES, mean = 68, sd = 4), # not sure about the SD = 4
  # for those without SD Rs, randomly sample from global Rs estimates, with replace=TRUE
  Rs1 = sample(var_Rs$Rs, N_SAMPLES, replace = TRUE), 
  # for those studies reported Rs (mean and SD, 9 studies) using rnorm functions
  Rs2 = rnorm(n = N_SAMPLES, mean = 80.4, sd = 16.9), # for those with standard deviation, N_SAMPLES samples were take from normal distribution ~ (mean=80.4, sd=16.9)
  # repeat until all Rs estimates with sd samples were taken and hold in the outputs
  Rs3 = rnorm(n = N_SAMPLES, mean = 98, sd = 12),
  Rs4 = rnorm(n = N_SAMPLES, mean = 79, sd = 7.65),
  Rs5 = rnorm(n = N_SAMPLES, mean = 94.4, sd = 4.59),
  Rs6 = rnorm(n = N_SAMPLES, mean = 91, sd = 2.05),
  Rs7 = rnorm(n = N_SAMPLES, mean = 94.3, sd = 9.13),
  Rs8 = rnorm(n = N_SAMPLES, mean = 78.34, sd = 2.25),
  Rs9 = rnorm(n = N_SAMPLES, mean = 72.55, sd = 7.13),
  Rs10 = rnorm(n = N_SAMPLES, mean = 93.3, sd = 3.11),
  
  # Step 2. Prepare GPP (same logic as global Rs)
  gpp1 = rnorm(n = N_SAMPLES, mean = 124.6, sd = 2.7), # if gpp have mean and standard deviation, take N_SAMPLES samples from the normal distribution
  gpp2 = rnorm(n = N_SAMPLES, mean = 135.7, sd = 21.712),
  gpp3 = rnorm(n = N_SAMPLES, mean = 109.29, sd = 27.33),
  gpp4 = rnorm(n = N_SAMPLES, mean = 118, sd = 26),
  gpp5 = rnorm(n = N_SAMPLES, mean = 119, sd = 6),
  gpp6 = rnorm(n = N_SAMPLES, mean = 110.5, sd = 21.3),
  gpp7 = rnorm(n = N_SAMPLES, mean = 117, sd = 13),
  gpp8 = rnorm(n = N_SAMPLES, mean = 147, sd = 16),
  gpp9 = rnorm(n = N_SAMPLES, mean = 119, sd = 6),
  gpp10 = rnorm(n = N_SAMPLES, mean = 122, sd = 25),
  # for those do not have SD information, sample using boosting 
  gpp11 = sample(var_GPP, N_SAMPLES, replace = TRUE) 
) ->
  bootstrap_data
...
bootstrap_data %>% 
  transmute(
    ...
    Rs_raw = rowMeans(select(bootstrap_data, starts_with("Rs"))),
    # calculate GPP, this is the Top-dowm GPP (collected from previous studies, and resampling)
    GPP_raw = rowMeans(select(bootstrap_data, starts_with("gpp")))
  ) ->
  bootstrap_data
```
...
gpp_qt <- quantile(simple_ratio_results$GPP, c(0.025, 0.5, 0.975)) %>% round(0)
gpp_qt_raw <- quantile(simple_ratio_results$GPP_raw, c(0.025, 0.5, 0.975)) %>% round(0)
...
rsg_qt <- quantile(simple_ratio_results$Rs, c(0.025, 0.5, 0.975)) %>% round(0)
rs_qt_raw <- quantile(simple_ratio_results$Rs_raw, c(0.025, 0.5, 0.975)) %>% round(0)
