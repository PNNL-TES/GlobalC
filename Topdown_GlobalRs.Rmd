...
bootstrap_data <- tibble(Rs_raw = replace_calSD_and_generate(GlobalRs$Rs, GlobalRs$SD),
                         GPP_raw = replace_calSD_and_generate(GPP$GPP, GPP$SD))

sd(bootstrap_data$Rs_raw) # why sd so small, not as aspected
sd(bootstrap_data$GPP_raw) # why sd so small, not as aspected
```
...
p1_gpp3 <- ggdraw() +
  ...
  draw_plot(p_gpp_marg, x = 0.35, y = 0.6, width = 0.4, height = 0.5) 
print(p1_gpp3)
...
bootstrap_data %>% 
  mutate(
    ...
    Fire = replace_calSD_and_generate(var_Fire$Amount_Pg, var_Fire$SD), # Another way to do fire resample (use mean and SD)
...
p1_rsg3 <- ggdraw() +
  ...
  draw_plot(p_gpp_marg, x = 0.55, y = 0.6, width = 0.4, height = 0.5) 
print(p1_rsg3)
...
cmip5_data_file <- "data/CMIP/1.cmip5_global_monthly_means.csv"
cmip6_data_file <- "data/CMIP/1.cmip6_global_monthly_means.csv"
if(file.exists(cmip5_data_file) && file.exists(cmip6_data_file)) {
  read.csv(cmip5_data_file, stringsAsFactors = FALSE) %>% 
    bind_rows(read.csv(cmip6_data_file, stringsAsFactors = FALSE)) %>% 
    as_tibble() %>% 
