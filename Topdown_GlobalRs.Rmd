...
rs_figure_data %>% 
  ...
  # scale_fill_discrete("", labels = c(expression(Canonical~Rs), expression("Rs"[GPP]~"          "))) +
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) + geom_point(alpha = 0.05) + geom_density2d() +
  scale_color_discrete("", labels = c(expression(Canonical~GPP~vs~Rs[GPP]), 
                                                            expression(Canonical~R[S]~vs~GPP[Rs]))) +
...
print(p_joint)
ggsave("outputs/Figure3_not_sure.png", width = 5, height = 4)
```
