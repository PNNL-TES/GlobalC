...
```{r load data}
# input data
read_file <- function(x) read.csv(file.path(DATA_DIR, x))
...
GPP <- read_file('GlobalGPP_Sum.csv') # 65 GPP estimates from published paper
GPP %>% filter(Year > 1980) -> GPP
NPP <- read_file('ITONPP.CSV') # 251 NPP estimates from ITO (2011)
GlobalRs <- read_file('GlobalRs.csv') # 27 estimates of global Rs
...
# There are 17 IGBP PFTs; map them to SRDB ecosystem types
igbp_pft <- c("Water" = "WAT", 
              "Forest" = "ENF", "Forest" = "EBF", "Forest" = "DNF", "Forest" = "DBF", "Forest" = "MF", 
              "Shrubland" = "CSH", "Shrubland" = "OSH",
              "Savanna" = "WSA", "Savanna" = "SAV",
              "Grassland" = "GRA", "Wetland" = "WET",
              "Agriculture" = "CRO", "Urbland" = "URB", "Agriculture" = "CVM", 
              "Snow" = "SNO", "Desert" = "BSV")
IGBP$IGBP <- igbp_pft[IGBP$IGBP2001Pr + 1]
IGBP$Ecosystem <- names(igbp_pft)[IGBP$IGBP2001Pr + 1]

igbp_group <- c("WAT", "EF", "EF", "DF", "DF", "MF", "SH", "SH", "GRA",
            "GRA", "GRA", "WET", "CRO", "URB", "CRO", "SNO", "BSV")
IGBP$IGBP_group <- igbp_group[IGBP$IGBP2001Pr + 1]
...
# GPP ~ NPP
ggplot(gpp_results, aes(x = NPP, y = GPP2)) +
  geom_hex() +
...
# GPP ~ Rs
ggplot(gpp_results, aes(x = gpp_results$Rs_raw, y = GPP2)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
...
# GPP ~ Rc
ggplot(gpp_results, aes(x = Rc, y = GPP2)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
  xlab(expression(RC)) +
...
# GPP ~ Froot
ggplot(gpp_results, aes(x = Froot, y = GPP2)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
...
p_gpp3_tt <- t_test(gpp_results$GPP_raw, simple_ratio_results$GPP)
```
...
3 (simple global Rs:GPP) | `r p_gpp3 * 100` %     | `r p_gpp3_tt`
...
var_RaGpp_mf <- var_RaGpp %>% filter(Ecosystem == "Forest") %>% pull(RaGPP_ratio)
var_RaGpp_gra <- var_RaGpp %>% filter(Ecosystem == "Grassland") %>% pull(RaGPP_ratio)
```
...
# Rs ~ GPP
ggplot(rs_results, aes(x = GPP_raw, y = Rs_topdown)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
  xlab(expression(GPP~(Pg~C~yr^{-1}))) +
...
# Rs ~ NPP
ggplot(rs_results, aes(x = NPP, y = Rs_topdown)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
  xlab(expression(NPP~(Pg~C~yr^{-1}))) +
...
# Rs ~ RaGpp
ggplot(rs_results, aes(x = RaGpp, y = Rs_topdown)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
...
# Rs ~ Froot
ggplot(rs_results, aes(x = Froot2, y = Rs_topdown)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
  xlab(expression(Froot)) +
...
# Rs ~ Fire
ggplot(rs_results, aes(x = Fire, y = Rs_topdown)) +
  geom_hex() +
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
...
p_Rs3_tt <- t_test(rs_results$Rs_raw, simple_ratio_results$Rs)
```
...
w <- c(0.25, w) %>% unique() %>% sort()  # make sure 0.25 is included
w <- c(0.5, w) %>% unique() %>% sort()  # make sure 0.5 is included
...
p_joint <- ggplot(results, aes(w, q500, color = which)) +
  ...
  geom_label(data = results_w50, aes(x = 0.1, y = q500, label = round(q500, 0)),
             size = 3, fontface = "bold", show.legend = FALSE) +
  
  # 25%
  geom_segment(data = results_w25, aes(x = 0, xend = w, yend = q500), 
               linetype = 2, size = 0.5) +
  geom_label(data = results_w25, aes(x = 0.05, y = q500, label = round(q500, 0)),
