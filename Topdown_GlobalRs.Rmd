...
gpp_results %>%
  select(`GPP[Rs]` = GPP2, `GPP` = GPP_raw) %>% 
  gather(GPP_type) ->
  gpp_figure2_data
...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown, `Rs` = Rs_raw) %>% 
  gather(Rs_type) ->
  rs_figure_data
...
rs_results %>%
  select(`Rs[GPP]` = Rs_topdown2, `Rs` = Rs_raw) %>% 
  gather(Rs_type) ->
  rs_figure_data
