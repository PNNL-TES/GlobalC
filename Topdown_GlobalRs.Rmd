...
GPP %>% ggplot(aes(x = Group, y = GPP, fill = as.factor(Group))) +
  ...
  ylab(expression(GPP ~ "(Pg C" ~ yr^{
    -1
  } ~ ")"))

ggsave("outputs/FigureSX.jpg", width = 8, height = 5, dpi = 300, units = "in")

GPP %>% ggplot(aes(x = Group, y = GPP)) +
  geom_point() +
  xlab("GPP by different groups") +
  ylab(expression(GPP ~ "(Pg C" ~ yr^{
    -1
...
# pdf("outputs/Figure S8 - Boots GPP pairs.pdf", width = 8, height = 6)
...
ggplot(rs_results, aes(x = Ra1)) +
  ...
  xlab(expression(Ra1 ~ or ~ Ra3 ~ "(" ~ Pg ~ C ~ yr^{
    -1
  } ~ ")")) +
...
ggsave("outputs/FigureS5_RA_Method.jpg", width = 8, height = 4, dpi = 300, units = "in")
...
ggplot(rs_results, aes(Fire, Rs_topdown)) +
  ...
  scale_fill_gradient(low = "gray", high = "blue") +
  geom_smooth(method = "lm") +
  xlab(expression(Fire ~ burned ~ (Pg ~ C ~ yr^{
    -1
  }))) +
  ylab(expression(R[S] ~ (Pg ~ C ~ yr^{
    -1
...
# pdf("outputs/Figure S9 - Boots Rs pairs.pdf", width = 8, height = 6)
...
p_joint <- ggplot(joint, aes(GPP, Rs, color = Source)) +
  ...
  scale_color_discrete("", labels = c(
    expression(Canonical ~ GPP ~ and ~ R[S] ~ implied ~ by ~ GPP ~ "(" ~ Rs[GPP] ~ ")"),
    expression(Canonical ~ R[S] ~ and ~ GPP ~ implied ~ by ~ R[S] ~ "(" ~ GPP[Rs] ~ ")")
  )) +
...
print(p_joint)
ggsave("outputs/Figure3_not_sure.png", width = 8, height = 6)
```
...
if (file.exists(cmip6_global_data_file)) {
  ...
  model_global_ratios %>%
    mutate(
      Scale = "Global scale",
      Source = paste0("CMIP6\n(N = ", length(unique(model_global_ratios$model)), ")"),
      Type = "Models"
    ) ->
  cmip_global
} else {
...
if (file.exists(cmip6_local_data_file)) {
  ...
  cmip6_local_data %>%
    select(experiment, model, Longitude, Latitude) %>%
    distinct() %>%
    nrow() ->
  samples
