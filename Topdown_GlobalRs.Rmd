...
# I think we want to take the *weighted* mean of two PDFs
n_points <- 20
...
rs_wm  <- tibble(q025 = rep(NA_real_, n_points), 
                 q500 = rep(NA_real_, n_points),
                 q975 = rep(NA_real_, n_points))
for(i in seq_along(w)) {
  weighted_gpp <- gpp_results$GPP_raw * w[i] + gpp_results$GPP * (1 - w[i])
  gpp_wm[i,] <- quantile(weighted_gpp, probs = QUANTILES)
  weighted_rs <- rs_results$Rs_raw * (1 - w[i]) + rs_results$Rs_topdown * w[i]
...
gpp_wm$which <- "GPP"
rs_wm$which <- "Rs"
results <- bind_rows(gpp_wm, rs_wm)
...
p_joint <- ggplot(results, aes(w, q500, color = which)) +
  ...
  coord_cartesian(ylim = c(40, 180), expand = FALSE) +
  ...
  xlab("<-- Determined by bottom-up Rs                    Determined by top-down GPP -->") +
  ylab(expression("50%"~quantile~(Pg~C~yr^-1)))
print(p_joint)
