...
sample_size_tst <- function(test_size, N_SAMPLES) {
  ...
  resample_RSG %>%
    ...
    ylab("Density") +
    xlab(expression(R[S]~(Pg~C~yr^{-1}))) -> RSG_sample_fig
  
  # resample gross primary production (GPP)
  resample_GPP <- tibble(GPP1_no_SD = sapply(rep(test_size, N_SAMPLES), method1, GPP$GPP),
  ...
  resample_GPP %>%
    ...
    scale_fill_discrete("", h.start = 180) + scale_color_discrete(h.start = 180) +
    ylab("Density") +
    xlab(expression(GPP~(Pg~C~yr^{-1}))) -> GPP_sample_fig
