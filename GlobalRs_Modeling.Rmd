...
## load data
```{r}
readd(mgrsd_climate) -> mgrsd_climate 
mgrsd_climate$EVI[mgrsd_climate$EVI < 0 & !is.na(mgrsd_climate$EVI)] <- 0

readd(global_land) -> global_land
```
...
R2_train <- summary(lm(Rs_Norm ~ prediction, data = train))$r.squared %>% round(5)
train %>% 
  ggplot(aes(prediction, Rs_Norm)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm") +
  geom_abline(slope = 1, intercept = 0, col = "red", linetype = "dashed") +
  ggtitle(paste0("Train data, R2=", R2_train, ", n=21,771(80%)")) +
...
R2_test <- summary(lm(Rs_Norm ~ prediction, data = test))$r.squared %>% round(5)
test %>% 
  ggplot(aes(prediction, Rs_Norm)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm") +
  geom_abline(slope = 1, intercept = 0, col = "red", linetype = "dashed") +
  ggtitle(paste0("Test data, R2=", R2_test, ", n=5,443(20%)")) +
  labs(x = expression(Predicted~R[S]~(g~C~m^{2}~day^{-1})),
       y = expression(Measured~R[S]~(g~C~m^{2}~day^{-1}))) ->
  test_plot
