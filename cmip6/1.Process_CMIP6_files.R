...
fx_files   <- read.csv(list.files(INPUT_DIR, pattern = '0.fx_files_CMIP6.csv', full.names = TRUE), stringsAsFactors = FALSE)
data_files <- read.csv(list.files(INPUT_DIR, pattern = '0.to_process_CMIP6.csv', full.names = TRUE), stringsAsFactors = FALSE)

# 2. Calculate the Land Area Weight .ncs -------------------------------------------------------
# Ensure that we are only processing the land area weights for the model, grid, experiment, and ensemble
# members we have access to.
fx_files %>%  
  left_join(data_files %>%  
              select(model, grid, experiment, ensemble) %>% 
              distinct) -> 
...
data_files %>% 
  full_join(LandAreaNc_tib %>% select(model, experiment, ensemble, grid, LandArea), 
            by = c("model", "experiment", "ensemble", "grid")) %>% 
...
cdo_regional_means <- function(data_LandArea, region_input = NULL){
  ...
  apply(data_LandArea, 1, function(input, region = region_input){
    ...
    base_name <- paste(input[['era']], input[["variable"]], input[["domain"]],  input[["model"]], input[["experiment"]],
                       input[["ensemble"]], input[["grid"]],input[["time"]], sep  = '_')
...
INTER_OUTPUT_CSV <- file.path(OUTPUT_DIR, 'intermediate-csv')
